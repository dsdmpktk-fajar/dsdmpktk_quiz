...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const syllabusStart = document.getElementById("syllabus-start");
    const syllabusEnd = document.getElementById("syllabus-end");
    const syllabusDuration = document.getElementById("syllabus-duration");
    ...
    function buildSyllabusSchedule(s) {
        ...
        return `
            <div class="small text-muted">
                ...
                ${start} — ${end} (${s.duration_minutes ?? 0} menit)
            </div>
        `;
    }
    buildSyllabusSchedule()
    ...
    window.editSyllabus = (id, title, desc, category, subcat, order) => {
        ...
        syllabusStart.value = start ? formatDateTimeLocal(start) : "";
        syllabusEnd.value = end ? formatDateTimeLocal(end) : "";
        syllabusDuration.value = duration ?? "";
    ...
    syllabusForm.addEventListener("submit", function (e) {
        ...
        const payload = {
            ...
            start_time: syllabusStart.value ? new Date(syllabusStart.value).toISOString() : null,
            end_time: syllabusEnd.value ? new Date(syllabusEnd.value).toISOString() : null,
            duration_minutes: syllabusDuration.value ? Number(syllabusDuration.value) : null
        };
