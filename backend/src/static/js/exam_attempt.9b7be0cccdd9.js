// exam_attempt_branching.js
// Full-featured frontend for exam attempt with branching support.
// Requires EXAM_ID and ATTEMPT_ID injected by template.
...
(function () {
  ...
  // Backend already filters branching when user_exam provided.
  // -----------------------
  async function loadQuestions() {
    try {
      const url = `/api/exam/exams/${EXAM_ID}/questions/?user_exam=${ATTEMPT_ID}`;
      const data = await jsonFetch(url);
      // data may be array or paginated; assume array
      ...
      // If backend includes 'user_answer' or 'existing_answer' fields, merge to local answers
      questions.forEach(q => {
        // try to prefill from question.user_answer if present
        if (q.user_answer) {
          // possible shapes: { selected_choices: [...], text_answer: "...", files: [] }
          answers[q.id] = {
      ...
      // ensure index is in-range: if current question disappeared, go to nearest
  ...
  // -----------------------
  // Timer handling (loads user_exam info from results endpoint)
  // -----------------------
  ...
  function renderQuestion() {
    ...
    // helper to escape choice text (if needed backend already safe)
    ...
    // restore any local answers known
    restoreAnswer(q);

    // attach listeners: for branching we want to save & reload when answer choice changes
    attachListeners(q);
    updateProgress();
  }
  ...
  function buildNavigation() {
    ...
    questions.forEach((q, i) => {
      ...
      btn.type = "button";
      btn.textContent = i + 1;
      // classes: current / answered / default
      if (i === index) btn.classList.add("current");
      const saved = answers[q.id];
      if (saved && (saved.selected_choices?.length || saved.text_answer?.trim() || saved.files)) btn.classList.add("answered");
      btn.onclick = () => {
        saveCurrent(); // persist local
        index = i;
        renderQuestion();
        buildNavigation();
      };
  ...
  function restoreAnswer(q) {
    ...
    // files: cannot prefill file inputs for security; show hint if set
  ...
  // On change we save that question (JSON submit) and reload questions to support branching.
  // -----------------------
  function attachListeners(q) {
    // radio/checkbox/dropdown -> immediate submit & reload to apply branching
    ...
    choiceInputs.forEach(el => {
      el.addEventListener("change", async () => {
        ...
        try {
          ...
          // reload questions from backend (branching will be reflected)
          const prevQid = q.id;
          await loadQuestions();
          // place user on the same question if still available, otherwise nearest
    ...
    // text area -> debounce autosave (we already have global autosave but also attach blur)
    const ta = qbox().querySelector("#text-answer");
    if (ta) {
      ta.addEventListener("blur", () => {
        saveCurrent();
        // do a small submit for this q
    ...
    // file input -> just update local answers (final submit will upload); optional immediate upload could be implemented
    const fi = qbox().querySelector("#file-answer");
    if (fi) {
  ...
  // This is used for immediate saves on choice changes and autosave.
  // -----------------------
  async function submitAnswers(answerPayloadArray = [], reloadAfter = true) {
    // answerPayloadArray : [{question: id, selected_choices: [], text_answer: "..."}]
    try {
  ...
  async function finalSubmit() {
    // build FormData: user_exam + answers JSON + file fields prefixed files_<qid>
    saveCurrent();
  ...
  function wireButtons() {
    ...
    document.getElementById("btn-finish").addEventListener("click", () => { finishFlow(); });
  ...
  window._exam_debug = {
    getState: () => ({ questions, index, answers })
  };
